...
- **Rate authoring form.** `Percentage` carries an exact `Rational` as the canonical string `"num/den"`, so a data file authoring VAT at 17.5% writes `"7/40"` — unreadable, and easy to get wrong, in hand-maintained tax data. Decimal→rational conversion is *exact*, so a `Percentage.Parse` that accepts both forms keeps the data legible (`rate: 0.175`) while losing nothing. What the parse accepts, and the canonical form it normalises to, are **spec rules** — so this wants deciding, not defaulting. Blocks `c5n.plan.md` step 2.1.
