...
**The deeper reason (and the standing answer to "why not OpenAPI / JSON Schema + an off-the-shelf generator?"):** those standards describe the **shape** of a payload; they have no vocabulary for **behaviour over** that shape — cross-field and cross-type validation, discriminator defaults and their runtime resolution, capability metadata, patch semantics. In a model where the value lives in those rules, a shape-only standard can describe the types and none of what matters, so the generated client gets types without semantics — precisely the part that must not be allowed to diverge. Reach for the standard first and adopt it if it fits; own a small IDL only where it demonstrably cannot express the semantics. *(Ordering matters for the argument as much as for the engineering: tried-then-built is judgment, built-then-justified is not-invented-here.)*

**The schema declares and names; it never expresses.** No conditionals, no loops, no templating, no expression language — a rule appears as `postcodeMatchesCountry`, the *name* of a function hand-written once per target and conformance-tested, and c5n emits a call to it. This is the constraint, not an omission: config formats that grew expression languages are why the instinct against "YAML as code" exists at all, and the line is what separates a schema file from Helm. It also keeps c5n domain-blind — a new rule is a new function, never a change to the engine.

**Why not protobuf or GraphQL SDL, which is the harder question than OpenAPI.** Both have a real IDL and mature multi-language codegen, so the shape-versus-behaviour argument alone does not dispose of them. The answer is what the generated code *is*: **protobuf generates data holders you wrap; c5n generates a typed boundary over types you already own.** `Currency.GBP` is a constant referencing a hand-written `Currency` that carries its own behaviour — protobuf has no way to say "construct this through that factory", no vocabulary for cross-field validation, and a binary wire format where f8n deliberately chose a readable one. Adopting its IDL would also mean inheriting field numbers and its optionality model, and taking a real parser dependency against the "≈ just `yaml.v3`" property. Considered and rejected for stated reasons, which is the whole difference from not considered.

**Authoring: YAML leads; code-first stays designed and available.** c5n reads YAML and **how the YAML gets there is the author's business** — that is the point of an interchange format rather than a language anyone is asked to adopt. For *this* repo the schema is hand-written, because there is no existing domain to preserve: f8n's types are hand-written primitives, not a model, so schema-first costs nothing to adopt while code-first costs an emitter that would buy DX for a domain that does not exist. **For a brownfield codebase the answer inverts** — transcribing an existing C# domain into YAML is a bad trade, and the emitter pays for itself the moment it removes hand-written mapping. **Trigger for revisiting: a real domain arriving that already exists in code.** *(The conformance claim is largely independent of this choice, which is worth stating because it is tempting to argue otherwise: types cannot diverge under either scheme since both are derived, and the behaviour that can diverge — parsers, validators — is hand-written twice and pinned by vectors regardless of where the schema came from.)*

Every type is one uniform declaration = **fields + a construction recipe**; the only axis is whether c5n **emits the type** or merely **constructs/references** it:
...
Country:                    # external; no emit: → positional-ctor *convention* (ctor args = fields)
  ...
  fields: { alpha2: string, alpha3: string, numeric: int, name: string, callingCode: int,
            defaultCurrency: Currency, primaryTz: string }
...
type: table<Country>
items:
  - { alpha2: GB, alpha3: GBR, numeric: 826, name: "United Kingdom", callingCode: 44, defaultCurrency: GBP, primaryTz: Europe/London }
  - { alpha2: FR, alpha3: FRA, numeric: 250, name: "France",         callingCode: 33, defaultCurrency: EUR, primaryTz: Europe/Paris }
```
...
**Why this shape wins (perf + conformance):** parsing the DSL to *data at build time* (vs a runtime resolver re-parsing each call) removes per-call parse cost; a *single* hand-written interpreter per language (vs inlining each message body) keeps the **conformance surface** tiny — the dominant axis for l10n's verified-cross-language north star. That is codegen's *grain*: emit data + a thin typed surface, keep logic in a shared runtime. (The general form: retrofitting codegen onto a hand-first design tends to land on the conventional resolver shape, because the hand-written structure is already fixed by the time generation is added and generation can only wrap it. Designing *for* codegen from day one selects this structure instead — **codegen-native by construction**.)

> **Held as candidate; the pressure test has now run.** The concrete l10n code-shape (typed shim + AST-as-data + a `render` interpreter) is a codegen-native **redesign**, deliberately diverging from the conventional translation-client / string-resolver structure — well-reasoned, and the reason the hold existed was that it had not been walked against a real message set. **`../l10n/codegen-shape-test.md` has walked all fifteen cases** (finished 2026-08-29). None forced new machinery; three *removed* something. The gate can lift: its deliverable — an eleven-entry requirements list for `tree<T>` and typed-shim emit — now exists, and the **only unspecified piece is the `tree<T>` declaration itself**, which is the next thing to write rather than a reason to keep waiting. `candidate` remains on the l10n doc for a narrower reason: six cases ran on invented stand-ins rather than a real corpus. That bears on whether the *message shapes* were representative, not on whether the engine requirements are sound. Full l10n structure → `../l10n/DESIGN.md`.
>
> **Requirements the walk produced**, in the order they were found: two trees (locale-neutral shims over per-locale data); a **flat** parts list, never a tree; shim-level dispatch on a generated enum; a parameter node carrying a default; **writers own output layout**, not just syntax; constants as a separate **value**-leaf emission path; **locale as an explicit parameter of the interpreter** (`render(data, args, locale)` is the conformance boundary — nothing below it reads ambient culture, everything above it is free to be idiomatic, which is where **the emitter takes the pain** finds its second limit); per-locale data carrying CLDR patterns and a pre-reduced plural rule table alongside messages; **dispatch declared, not inferred**; sole-literal messages lowering to a bare string with leaf *kind* declared rather than inferred from arity; and every entry tagged with its source locale. Plus one requirement that is not c5n's: the escape bug lives in the **emitter**, above the boundary vectors drive, so its fixtures must start from DSL **source**.
...
## Change log
- 2026-08-29: **the l10n pressure test finished, so the `tree<T>` / typed-shim hold lifts.** All fifteen cases in `../l10n/codegen-shape-test.md` are resolved; none forced new machinery mid-walk and three removed something. The deliverable the hold was waiting for — a requirements list for both capabilities — is now eleven entries, and the only unspecified piece is the **`tree<T>` declaration itself**, which is the thing to write next rather than a reason to keep waiting. The entry that reaches furthest into this doc is the **conformance boundary**, `render(data, args, locale)`: locale is an explicit parameter, nothing below the boundary reads ambient culture, and above it each language may obtain the locale idiomatically. That gives **the emitter takes the pain, not the consumer** its second stated limit — the first being the wire format — and it is the same shape: idiom governs everything the harness does not have to compare. Also generalised from two independent findings: **structural facts are declared, never recovered by lookup** (dispatch, leaf kind, parameter optionality), because inference lets an unrelated edit change an API silently where a declaration fails the build by name.
