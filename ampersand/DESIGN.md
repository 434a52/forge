...
- **The payload is a second surface, and it gets the same treatment.** *"Client and server can't disagree about the primitives"* is proven for **values** by the golden vectors. The **bytes between them** are a separate claim: a `Money` serialised by the C# API and read by the Vue client must be the same value, and the same bytes back again. ampersand is the first place in the stack where a real payload crosses C# → TS, so it is where that stops being theoretical. It inherits the house rule (`../CLAUDE.md`): **both round trips** — `fromJson(toJSON(x)) == x` and `toJSON(fromJson(w)) == w` — asserted in both languages, in ampersand's own suite. It does **not** need `f8n`'s language-neutral dataset and driver; those exist so a third party can audit a published contract, and ampersand's two halves are versioned and deployed together. *(The sweep response is the one to watch — the largest thing crossing, and the client re-reads it on every drag.)*
...
## Change log
- 2026-08-27: **the client/server payload is named as its own conformance surface.** The architecture already claimed client and server cannot disagree about the primitives, which the golden vectors prove for *values*; the **bytes between them** were an unstated second claim. ampersand is the first place in the stack where a real payload crosses C# → TS, so it inherits the house round-trip rule — both directions, both languages, in its own suite — without inheriting `f8n`'s neutral dataset and driver, which exist for third-party audit of a published contract rather than for two halves shipped together.
